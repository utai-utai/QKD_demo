...
def stage_two_loss(student_logits: Tensor, teacher_logits: Tensor, labels: Tensor, temperature: float, top_k: int) -> dict[str, Tensor]:
    """计算 0.5 CE + 0.5 tau² KD。"""
    ce = torch.nn.functional.cross_entropy(student_logits[:, :-1].reshape(-1, student_logits.size(-1)),labels[:, 1:].reshape(-1), ignore_index=-100)
    kd = expanded_topk_logit_kl(student_logits, teacher_logits, labels, temperature, top_k)
    return {"loss": 0.5 * ce + 0.5 * kd, "ce": ce, "kd": kd}


def expanded_topk_indices(teacher_logits: torch.Tensor, labels: torch.Tensor, top_k: int) -> torch.Tensor:
...
def expanded_topk_logit_kl(student_logits: torch.Tensor, teacher_logits: torch.Tensor, labels: torch.Tensor, temperature: float = 2.0, top_k: int = 64) -> torch.Tensor:
    ...
    candidate_mask = torch.cat([torch.ones_like(present).expand_as(indices[..., :topk_size]), ~present], dim=-1)
    ...
    per_token = F.kl_div(F.log_softmax(student, -1), F.softmax(teacher, -1), reduction="none").sum(-1)
