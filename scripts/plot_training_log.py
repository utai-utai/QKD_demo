...
def plot_training_logs(run_selectors: Iterable[str], metrics: list[str]) -> Path:
    """绘制同一 Stage 的指定运行与指标，并返回保存至 analysis 的图片路径。"""
    selections = list(run_selectors)
    if not selections:
    ...
    if len(logs) == 1:
        _plot_one_run(axis, logs[0], metrics)
    else:
        _plot_multiple_runs(axis, logs, metrics)
    axis.set_title(f"{stage} training metrics")
    axis.set_xlabel("step")
    axis.set_ylabel("metric value")
    axis.grid(alpha=0.25)

    output = _output_path(stage, [selector for selector, _ in runs], metrics)
...
def _plot_one_run(
    ...
    for index, metric in enumerate(metrics):
        axis.plot(
            steps,
            [_float_or_nan(row[metric]) for row in rows],
...
def _plot_multiple_runs(
    ...
    axis.legend(
        handles=[Line2D([0], [0], color=colors(index % colors.N), label=run.name) for index, (_, run, _, _) in enumerate(logs)],
        title="run",
    )
    style_key = "  ".join(
...
def _resolve_run(selector: str) -> Path:
    ...
    exact = [path for path in candidates if path.name == normalized]
    return exact[0] if exact else max(candidates, key=lambda path: path.name)
...
def _output_path(stage: str, selectors: list[str], metrics: list[str]) -> Path:
    selection = _layer_filename(selectors)
    metric_part = "-".join(_filename_part(metric) for metric in metrics)
    return _ANALYSIS_ROOT / stage / f"{selection}_{metric_part}.png"
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        metavar="RUN",
        help="运行名，如 layer-21 或 layers-21-22-23；省略 rank/日期时自动选择最新运行",
    )
    parser.add_argument(
        "--metrics",
        nargs="+",
...
def main() -> None:
    args = arguments()
    output = plot_training_logs(args.runs, args.metrics)
